 ...
 * ------------------
 *
 * There are two types of simple string expressions:
 *
 ...
 *     the value of the expression.  We use deferred literals, rather than in-
 *     place string literals, because they can be translated independently of
 *     the expression.
 *
                             ...
                             gint          depth)
{
  ...
  gimp_prop_eval_read_token (&expr, &t);
  ...
  else if (gimp_prop_eval_is_name (*t))
    {
      ...
      if (g_type_is_a (type, G_TYPE_BOOLEAN))
      ...
      else if (g_type_is_a (type, G_TYPE_ENUM))
        {
          ...
          if (g_strcmp0 (gimp_prop_eval_read_token (expr, t), "{"))
            {
              g_set_error (error, GIMP_PROP_EVAL_ERROR, GIMP_PROP_EVAL_FAILED,
          ...
          while (gimp_prop_eval_read_token (expr, t) &&
  ...
  if (! g_strcmp0 (gimp_prop_eval_read_token (expr, t), "."))
    {
      ...
      if (! gimp_prop_eval_read_token (expr, t) ||
          ! gimp_prop_eval_is_name (*t))
        {
          g_set_error (error, GIMP_PROP_EVAL_ERROR, GIMP_PROP_EVAL_FAILED,
                       "invalid reference");
